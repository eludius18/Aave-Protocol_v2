...
/**
 * @title SafeERC721
 * @dev Wrappers around ERC721 operations that throw on failure (when the token
 * contract returns false). Tokens that return no value (and instead revert or
 * throw on failure) are also supported, non-reverting calls are assumed to be
 * successful.
 * To use this library you can add a `using SafeERC721 for IERC721;` statement to your contract,
...
library SafeERC721 {
  ...
  function safeApprove(
    ...
    require(
      ...
      'SafeERC721: approve from non-zero to non-zero allowance'
    );
